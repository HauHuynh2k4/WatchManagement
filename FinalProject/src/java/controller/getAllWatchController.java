 ...
 * @author ADMIN
 */
@WebServlet(name = "getAllWatchController", urlPatterns = {"/getAllWatchController"})
public class getAllWatchController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<WatchDTO> listWatch = dao.getAllWatch();
            if (listWatch != null && !listWatch.isEmpty()) {
                session.setAttribute("LIST_WATCH", listWatch);
                request.setAttribute("LIST_WATCH", listWatch);
                if(user.getRole().equals("AD")){
                    url=ADMIN_PAGE;
                }
            } else {
                request.setAttribute("ERROR", "No watches found");
            }
        } catch (Exception e) {
